...
def get_selected_classes_points(config: DictConfig,
    ...
    if CLASSIFICATION_STR in fields_to_keep:
        fields_to_keep.remove(CLASSIFICATION_STR)
...
def get_complementary_points(config: DictConfig) -> pd.DataFrame:
    ...
    with laspy.open(donor_file_path) as donor_file, \
        ...
        df_recipient_non_empty_patches = df_recipient_points.groupby(by=[PATCH_X_STR, PATCH_Y_STR]).count().classification == 0
